...
# MYu, 2020-Sept-06, Added code to save/load data from text file
#------------------------------------------#
...
class FileIO:
    ...
    def save_inventory(file_name: list, lst_Inventory: list) -> None:
        ...
        try:
            ...
            # Writing CD data to text file
            with open(file_name_CD, 'w') as file:
                for disc in lst_Inventory:
                    file.write(disc.get_record())

            # Writing track data to text file. Track data is mapped to corresponding CD album
    ...
    def load_inventory(file_name: list) -> list:
        ...
        try:
            ...
            # Reading CD data from text file and load to Python runtime
            ...
            # Reading track data from text file and load to corresponding CD object
            with open(file_name_track, 'r') as file:
                for line in file:
                    for cdAlbum in lst_Inventory:
                        data = line.strip().split(',')
                        # Load data to CD object only if CD object ID in runtime matches CD object ID from text file
                        if cdAlbum.cd_id == int(data[0]):
                            trackRow = DC.Track(int(data[1]), data[2], data[3])
                            cdAlbum.add_track(trackRow)

        # Error handling for initial start of program, where text files do not exist
